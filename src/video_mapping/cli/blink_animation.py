...
def _hsv_to_rgb(h: float, s: float, v: float) -> RGBColor:
    ...
    if s <= 1e-12:  # s == 0 check for floats (with some epsilon)
        vi = int(v * 255)
        return (vi, vi, vi)
    h6 = h * 6.0
    i = int(h6) % 6
...
def _pane(layout: Layout, building_row: int, pane_row: int, col: int) -> Pane:
    """Return the pane for a logical (row, column) position."""
    return layout.rows[building_row].blocks[col // 6].halves[(col % 6) // 3].pane_at(pane_row, col % 3)


def _draw_wave_field(canvas: Canvas, layout: Layout, t: float) -> None:
    """Draw the base moire wave layer with slow hue drift.

    Combine three sine fields at different orientations to get an interference
    pattern. The global hue rotates over about 20 seconds.
    """
    global_hue = t / 20.0
...
def _draw_horizontal_scan_beam(canvas: Canvas, layout: Layout, t: float) -> None:
    """Draw a soft white beam that ping-pongs vertically every 2.5s."""
...
def _draw_block_strobe(canvas: Canvas, layout: Layout, t: float) -> None:
    """Strobe mirrored blocks with a travelling phase wave.

    Each block gets a phase offset, then flashes only near sine peaks to keep
    the strobe sparse and punchy.
    """
...
def _draw_sparkles(canvas: Canvas, layout: Layout, t: float, rng: np.random.Generator) -> None:
    """Scatter warm sparkles with density modulated by a slow pulse."""
...
def _draw_pillar_bars(
    ...
    *,
    ...
    """Render organic pillar bars and fade them out during the end scan."""
    if not draw_pillar_bars:
        return

    pillar_remaining = 1.0 if end_scan_progress is None else max(0.0, 1.0 - end_scan_progress)
...
def _radial_alpha_mask(
    ...
    *,
    ...
    """Build a clipped radial alpha mask for a local glow region."""
    ...
    return np.clip((1.0 - dist) * alpha, 0.0, 1.0).astype(np.float32)


def _blend_region_with_alpha(region: np.ndarray, color: RGBColor, local_alpha: np.ndarray) -> None:
    """Blend a solid color over RGB/RGBA pixels using per-pixel alpha."""
...
def _blend_radial_glow(
    ...
    *,
    ...
    """Blend one radial glow contribution into the canvas buffer."""
    ...
    local_alpha = _radial_alpha_mask(x1, y1, x2, y2, cx=cx, cy=cy, radius=radius, alpha=alpha)
    if np.all(local_alpha <= 0.0):
        return

    region = canvas_arr[y1 : y2 + 1, x1 : x2 + 1]
    _blend_region_with_alpha(region, color, local_alpha)


def _apply_end_scan_shutdown(
    canvas: Canvas,
    base: Canvas,
    end_scan_panes: list[Pane],
    end_scan_progress: float | None,
) -> None:
    """Retire panes in scan order and highlight the active shutdown pane."""
    ...
    _blend_radial_glow(canvas, canvas_arr, (188, 218, 255), cx=center_x, cy=center_y, radius=30, alpha=0.12)
    _blend_radial_glow(canvas, canvas_arr, (220, 238, 255), cx=center_x, cy=center_y, radius=18, alpha=0.16)
    _blend_radial_glow(canvas, canvas_arr, (242, 248, 255), cx=center_x, cy=center_y, radius=10, alpha=0.20)
...
def _render_frame(
    ...
    *,
    ...
    """Render one frame by composing the animation layers in order."""
    _draw_wave_field(canvas, layout, t)
    _draw_horizontal_scan_beam(canvas, layout, t)
    _draw_block_strobe(canvas, layout, t)
    _draw_sparkles(canvas, layout, t, rng)
    _draw_pillar_bars(
        canvas,
        layout,
        t,
        draw_pillar_bars=draw_pillar_bars,
        end_scan_progress=end_scan_progress,
    )
    _apply_end_scan_shutdown(canvas, base, end_scan_panes, end_scan_progress)
